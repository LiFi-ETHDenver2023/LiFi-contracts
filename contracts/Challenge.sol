...
contract Challenge {
    ...
    */
    ...
    mapping(ChallengeDataId => ChallengeData) private _lifiChallenges;
    ...
    function createChallenge(
        ...
        _lifiChallenges[challengeDataId] = challengeData;
    ...
    function startChallenge(
        ...
        require(
            _lifiChallenges[challengeDataId].challengeStatus ==
                uint256(ChallengeStatus.CREATED),
            "Challenge: challenge must be in created state"
        );

        _lifiChallenges[challengeDataId].challengeStatus = uint256(
            ChallengeStatus.STARTED
        );
        _lifiChallenges[challengeDataId].startTime = block.timestamp;
    ...
    function finishChallenge(string memory challengeCodeId) external onlyHost(challengeCodeId) {
        ...
        _lifiChallenges[challengeDataId].endTime = block.timestamp;
    ...
    function joinChallenge(
        ...
        _lifiChallenges[challengeDataId].participants.push(msg.sender);
    ...
    function claimForChallengeReward(
        ...
        uint256 rewardAmount = _lifiChallenges[challengeDataId]
            .finalRewardForSuccessfulParticipants /
            _lifiChallenges[challengeDataId].participants.length;
